    ...
    public void GenerateGrid()
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < cols; col++)
            {
                ...
                circ.spriteRenderer = reference.GetComponent<SpriteRenderer>();
                circ.coll = reference.GetComponent<SphereCollider>();
                circ = circles[i].AddComponent<Circle>();
                i++;
            }
        }
